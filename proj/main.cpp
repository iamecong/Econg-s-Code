...
struct address_list
{
	...
	char handset[30];  //手机
	char email[30];    //电子邮件
	char address[30];  //个人介绍 
	struct address_list *next;
};
struct address_list *release(struct address_list *head); // 释放内存函数声明
//创建函数，不带头结点的链表
...
//排序函数
struct address_list *rank(struct address_list *head)
{
	struct address_list *p1,*p2;
	int i,j;
...
//释放内存函数
struct address_list *release(struct address_list *head)
{
...
void save(struct address_list *head)
{
	...
	p1=head;
	fprintf(fp,"姓名    学号      手机     Email     个人介绍\n");
	for(;p1!=NULL;) 
	{
